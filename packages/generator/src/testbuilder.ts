...
export class TestTxBuilder extends TxBuilder {
  ...
  build(): RawTx {
    ...
    const getTxFee = (): Fp => {
      ...
      logger.info(`this.feePerByte ${this.feePerByte} and total L2Fee : ${this.feePerByte.muln(size)}`)
    ...
    logger.info(`spendables ${spendables.length}, spendings ${spendings.length}`)
