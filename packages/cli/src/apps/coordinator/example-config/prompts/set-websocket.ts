  ...
  async run(
    ...
    do {
      ...
      try {
        ...
        const network = await webSocketProvider.detectNetwork()
        address = addressesByNetworkId[network.chainId.toString(10)]
        if (address) {
        ...
        const { confirm } = await this.ask({
          type: 'confirm',
          name: 'confirm',
          message: `Zkopru is not deployed on this network (${network.chainId}), continue?`,
        })
        if (confirm) {
          websocket = websocketUrl
          address = ''
